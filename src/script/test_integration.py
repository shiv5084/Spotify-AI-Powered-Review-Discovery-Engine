...
def test_api_dashboard():
    ...
    # Top-level keys
    required_top = {"week_ending", "pulse_note_text", "metrics"}
    ...
    ok(f"All 7 metrics keys present")
...
def test_root_redirect():
    section("Test 5 — GET / (root redirect to /dashboard)")
    # The root page.tsx does a server-side redirect so Next.js may return 200 directly
    # (redirect is handled server-side before response)
    code, body = http_get("/")
...
def main():
    ...
    print("=" * 60)
    print("  PRDE INTEGRATION TEST — Frontend + Backend API")
    print("=" * 60)
