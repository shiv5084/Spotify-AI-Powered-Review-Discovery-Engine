...
class ReviewProcessor:
    ...
    def process_reviews_optimized(
        ...
        if gate_enabled:
            ...
            # Auto-label gate-rejected reviews based on rating fallback
                    ...
                    if not success:
                        for j, global_idx in enumerate(chunk_global_indices):
                            try:
                                ...
                                rating = sampled_df.iloc[global_idx].get("rating")
                                results[global_idx] = self._fallback_analysis_dict(rating)

                    if tpd_halt:
                        break

            # Fill any None left from TPD halt or edge cases using rating fallback
                ...
                if not success:
                    for j in range(len(chunk_texts)):
                        try:
                            ...
                            rating = sampled_df.iloc[chunk_start + j].get("rating")
                            results[chunk_start + j] = self._fallback_analysis_dict(rating)
