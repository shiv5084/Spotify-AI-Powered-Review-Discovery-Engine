...
| Component | Role |
|---|---|
| `scrapers.py` | Source-specific fetchers for 6 channels: Google Play via `google-play-scraper` library (500 reviews), Apple App Store via iTunes RSS JSON feed, Reddit via `/r/spotify/new.json` feed, Spotify Forums and Twitter/X via Playwright browser automation, Trustpilot via BeautifulSoup HTML parsing. |
...
* **iTunes RSS feed instability**: The App Store customer reviews feed is undocumented and often returns empty payloads. The engine handles this gracefully and relies on the Google Play Store (500 reviews) + other sources to aggregate feedback.
